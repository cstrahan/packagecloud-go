...
[![fern shield](https://img.shields.io/badge/%F0%9F%8C%BF-Built%20with%20Fern-brightgreen)](https://buildwithfern.com?utm_source=github&utm_medium=github&utm_campaign=readme&utm_source=Cstrahan%2FGo)

The Cstrahan Go library provides convenient access to the Cstrahan APIs from Go.
...
```go
client := client.NewClient(
    option.WithBaseURL(packagecloud.Environments.Default),
)
```
...
```go
response, err := client.GpgKeys.Create(...)
if err != nil {
...
```go
response, err := client.GpgKeys.WithRawResponse.Create(...)
if err != nil {
    return err
}
...
response, err := client.GpgKeys.Create(ctx, ...)
```
...
response, err := client.GpgKeys.Create(ctx, request, ...)
```
