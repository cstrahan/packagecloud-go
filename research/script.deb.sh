...
main ()
{
  ...
  gpg_key_install_url="https://REDACTED_TOKEN:@packagecloud.io/install/repositories/USER/REPOSITORY/gpg_key_url.list?os=${os}&dist=${dist}&name=${unique_id}"
  apt_config_url="https://REDACTED_TOKEN:@packagecloud.io/install/repositories/USER/REPOSITORY/config_file.list?os=${os}&dist=${dist}&name=${unique_id}&source=script"
  ...
  apt_source_path="/etc/apt/sources.list.d/USER_REPOSITORY.list"
  apt_keyrings_dir="/etc/apt/keyrings"
  if [ ! -d "$apt_keyrings_dir" ]; then
    install -d -m 0755 "$apt_keyrings_dir"
  fi
  gpg_keyring_path="$apt_keyrings_dir/USER_REPOSITORY-archive-keyring.gpg"
    gpg_key_path_old="/etc/apt/trusted.gpg.d/USER_REPOSITORY.gpg"
  ...
  apt_auth_conf_path="/etc/apt/auth.conf.d/USER_REPOSITORY.conf"
  echo -n "Adding repository credentials to $apt_auth_conf_path..."
  apt_auth_conf_url="https://REDACTED_TOKEN:@packagecloud.io/install/repositories/USER/REPOSITORY/apt_auth_conf?os=${os}&dist=${dist}&name=${unique_id}"
  apt_auth_conf_contents=`curl -sSf "${apt_auth_conf_url}"`
  curl_exit_code=$?
  if [[ -n "$apt_auth_conf_contents" ]]; then
    echo "$apt_auth_conf_contents" > $apt_auth_conf_path
    echo "Setting file permissions so only owner has read & write access to configuration file..."
    chmod 0600 "${apt_auth_conf_path}"
  fi

  if [ "$curl_exit_code" -gt "0" ]; then
    echo
    echo "Unable to run: "
    echo "    curl ${apt_auth_conf_url}"
    echo
    echo "Double check your curl installation and try again."
    [ -e $apt_auth_conf_path ] && rm $apt_auth_conf_path
    exit 1
  else
    echo "done."
  fi
